...
static void par_shapes__normalize3(float* v) {
	float lsqr = sqrtf(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]);
	if (lsqr > 0) {
...
static void par_shapes__trefoil(float const* uv, float* xyz, void* userdata) {
	...
	float q[3];
	q[0] =
		-1.5f * b * sinf(1.5f * u) * cosf(u) - (a + b * cosf(1.5f * u)) * sinf(u);
	q[1] =
		-1.5f * b * sinf(1.5f * u) * sinf(u) + (a + b * cosf(1.5f * u)) * cosf(u);
	q[2] = 1.5f * c * cosf(1.5f * u);
	par_shapes__normalize3(q);
	float qvn[3] = {q[1], -q[0], 0};
	par_shapes__normalize3(qvn);
	float ww[3];
	par_shapes__cross3(ww, q, qvn);
	xyz[0] = x + d * (qvn[0] * cosf(v) + ww[0] * sinf(v));
	xyz[1] = y + d * (qvn[1] * cosf(v) + ww[1] * sinf(v));
	xyz[2] = z + d * ww[2] * sinf(v);
}
...
par_shapes_mesh* par_shapes_create_disk(float radius, int slices,
	...
	*points++ = 0;
	for (int i = 0; i < slices; i++) {
		...
		*points++ = radius * cosf(theta);
		*points++ = radius * sinf(theta);
		*points++ = 0;
	}
	...
	par_shapes_rotate(mesh, acosf(nnormal[2]), axis);
...
par_shapes_mesh* par_shapes_create_subdivided_sphere(int nsubd) {
	...
	mesh = par_shapes_weld(mesh, 0.01f, nullptr);

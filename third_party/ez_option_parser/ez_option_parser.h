...
public:
	static ezOptionParserIDGenerator& instance() {
		static ezOptionParserIDGenerator Generator; return Generator;
	}
	short next() {
		return ++_id;
	}
...
public:
	...
	enum OP {
		...
		INLIST /* if value is in list */
	};

	enum TYPE {
		NOTYPE = 0, S1, U1, S2, U2, S4, U4, S8, U8, F, D, T
	};
	enum TYPE2 {
		NOTYPE2 = 0, INT8, UINT8, INT16, UINT16, INT32, UINT32, INT64, UINT64, FLOAT, DOUBLE, TEXT
	};
...
#define VALIDATE(T, U, LIST) { \
  ...
  switch (op) {\
    ...
    case GTLE:\
      ...
      break;\
      case NOOP: case INLIST: default: break;\
  } \
  }

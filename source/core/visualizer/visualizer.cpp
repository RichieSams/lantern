...
Visualizer::Visualizer(FrameData *startingFrameData, std::atomic<FrameData *> *swapFrameData)
        : m_window(nullptr),
          m_allocator(nullptr),
          m_accumulationBuffer(startingFrameData->Width, startingFrameData->Height),
          m_currentIntegrationFrameData(startingFrameData),
          m_swapIntegrationFrameData(swapFrameData) {
	g_visualizer = this;
}
...
bool Visualizer::RenderFrame() {
	...
	m_currentIntegrationFrameData = std::atomic_exchange(m_swapIntegrationFrameData, m_currentIntegrationFrameData);

	if (!m_currentIntegrationFrameData->Empty) {
		for (uint32_t y = 0; y < m_currentIntegrationFrameData->Height; ++y) {
		...
		m_accumulationBuffer.Empty = false;
	}
	...
	// Copy m_accumulationBuffer data to the GPU
